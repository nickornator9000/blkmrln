...
from src.venvmanager.config_manager import ConfigManager
import getopt
import sys
...
def entrypoint(argv)->None:
    try:
        ...
        print(str(err))
        print("\nUsage: python -m venvmanager [-h] [-l] [-t] [--input=FILEPATH]")
        sys.exit(2)

    for opt, arg in opts:
        if opt in ("-h", "--help"):
            print("Usage: python -m venvmanager [-h] [-l] [-t] [--input=FILEPATH]")
            sys.exit()
        elif opt in ("-i", "--input"):
